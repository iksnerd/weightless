...
Search and browse the registry. Returns up to 100 results, newest first.
...
# Filter by category
curl "http://localhost:8080/api/registry/search?category=models"

# Combined
curl "http://localhost:8080/api/registry/search?q=weights&category=models&publisher=meta"

# Browse all (no params = recent 100)
curl "http://localhost:8080/api/registry/search"
```
