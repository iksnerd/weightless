...
### Step A: Generation (CLI or Next.js Hub)

**Via `wl` CLI:**
```bash
# Passkey is baked into the .torrent announce URL
TRACKER_SECRET=your-secret wl create --user-id alice --name "My Dataset" ./data/

# Passkey is used for announce when downloading
TRACKER_SECRET=your-secret wl get --user-id alice "magnet:?xt=urn:btih:..."
```

The CLI reads `TRACKER_SECRET` from env and `--user-id` from the flag (or `WL_USER_ID` env), signs the user ID, and constructs the announce URL: `http://tracker:8080/announce/alice.{hmac_sig}`
...
Standard BitTorrent clients (Transmission, qBittorrent, etc.) use whatever announce URL is in the `.torrent` file — the passkey is transparent to them.
