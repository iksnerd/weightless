...
func (s *SwarmState) LoadFromDB() error {
	...
	_ = DB.QueryRow("SELECT COUNT(*) FROM registry").Scan(&s.Registered)
...
func (s *SwarmState) FlushToDB() {
	...
	_, _ = tx.Exec("DELETE FROM peers") // Clear old state
	stmt, _ := tx.Prepare("INSERT INTO peers (info_hash, peer_id, addr, updated_at, left, downloaded, uploaded) VALUES (?, ?, ?, ?, ?, ?, ?)")

	for hash, swarm := range snapshot {
		for id, p := range swarm {
			_, _ = stmt.Exec(hash, id, p.Addr, p.UpdatedAt, p.Left, p.Downloaded, p.Uploaded)
		}
	}
...
func (s *SwarmState) DrainBacklog() {
	...
	req, _ := http.NewRequest("POST", hubURL+"/api/internal/usage-sync", bytes.NewBuffer(body))
	req.Header.Set("Content-Type", "application/json")
	if key := os.Getenv("REGISTRY_KEY"); key != "" {
		req.Header.Set("X-Weightless-Key", key)
	}
	...
	if err != nil || resp.StatusCode != http.StatusOK {
		return // Still down, try again next cycle
	}
	defer resp.Body.Close()
