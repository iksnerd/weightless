...
func TestAnnounceCompletionDBError(t *testing.T) {
	...
	// Drop the registry table to force a DB error on completion update
	DB.Exec("DROP TABLE registry")

	// Announce with event=completed should still return peers (graceful degradation)
	req := httptest.NewRequest("GET", buildAnnounceURL(v2Hash, "peer001", "6881", "event=completed", "left=0"), nil)
	req.RemoteAddr = "127.0.0.1:5000"
	w := httptest.NewRecorder()
	HandleAnnounce(w, req)
...
func TestAnnouncePasskeyErrorNotLeaked(t *testing.T) {
	db := setupTest(t)
	defer db.Close()
	...
	// Send a malformed passkey (no dot separator)
	u := buildAnnounceURL(v2Hash, "p1", "6881")
	u = "/announce/malformed_no_dot" + u[9:]
	...
	if strings.Contains(body, "invalid passkey format") {
		t.Error("Internal error string 'invalid passkey format' leaked to client")
	}
	if strings.Contains(body, "TRACKER_SECRET") {
		t.Error("Internal error string 'TRACKER_SECRET' leaked to client")
	}

	// Response should contain a generic unauthorized message
	var resp map[string]interface{}
	if err := bencode.DecodeString(body, &resp); err != nil {
		t.Fatalf("Failed to decode response: %v", err)
	}
	reason, _ := resp["failure reason"].(string)
	if reason != "Unauthorized" {
		t.Errorf("Expected generic 'Unauthorized', got %q", reason)
	}
}
